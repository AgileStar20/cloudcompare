var FormHandler =  {
	...
	try {
	    ...
	    $(form).find("input").map(function(i, field) {if(field.type.match(/text|email|tel|hidden|radio|checkbox/)) { fields_to_gather.push(field.name); }});
	    $(form).find("select").map(function(i, field) { fields_to_gather.push(field.name); });
	...
	return false;
		...
		handle_marketing_response: function(event) {
			...
			if(form.hasClass("redirect")) {
				dataLayer.push({'Asset Label': asset_label});
				dataLayer.push({'event': 'freetrial-submitted'});
				...
				dataLayer.push({'Asset Label': asset_label});
				dataLayer.push({'event': 'contactform-submitted'});
        ll_conversion("Subscribed to CC updates");
				$.magnificPopup.open({items: {src: '#thank-you-all'}});
			} else if(form.hasClass("selected-export")) {
				dataLayer.push({'Asset Label': asset_label});
				dataLayer.push({'event': 'contactform-submitted'});
        ll_conversion("Subscribed to CC updates");
				$.magnificPopup.open({items: {src: '#thank-you-selected'}});
			} else {
				dataLayer.push({'Asset Label': asset_label});
				dataLayer.push({'event': 'contactform-submitted'});
        ll_conversion("Subscribed to CC updates");
				$.magnificPopup.open({items: {src: '#thank-you'}});
			}
		},
		...
		handle_submit: function(event) {
			...
			if(form_name != "Contact_Us") {
				$.magnificPopup.open({items: {src: '#processing'}}); 
			}
